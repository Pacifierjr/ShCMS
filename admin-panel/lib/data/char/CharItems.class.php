...
Class CharItems{
	...
	public function readItems(){
		$this->sql = $this->conn->prepare("SELECT [i].[ItemID], CAST([i].[ItemUID] as varchar(32)) as ItemUID, [i].[Slot], [i].[Quality], [i].[Gem1], [i].[Gem2], [i].[Gem3], [i].[Gem4], [i].[Gem5], [i].[Gem6], [i].[Craftname], [i].[Count], [i].[MakeTime], [id].[ItemName], [id].[ReqLevel], [id].[Quality] AS [MaxQuality], [id].[Slot] AS [MaxSlot], [i].[Bag]
	...
	public function getItemOnSlot($slot){
		...
		$this->sql = $this->conn->prepare("SELECT [i].[ItemID], [i].[ItemUID], [i].[Slot], [i].[Quality], [i].[Gem1], [i].[Gem2], [i].[Gem3], [i].[Gem4], [i].[Gem5], [i].[Gem6], [i].[Craftname], [i].[Count], [i].[MakeTime], [id].[ItemName], [id].[ReqLevel], [id].[Quality] AS [MaxQuality], [id].[Slot] AS [MaxSlot],  [i].[Bag]					  FROM [PS_GameData].[dbo].[CharItems] AS [i]
					  INNER JOIN [PS_GameDefs].[dbo].[Items] AS [id] ON [id].[ItemID] = [i].[ItemID]
	...
	public function deleteItemOnSlot($slot){
		...
		$this->sql = $this->conn->prepare("SELECT [ItemUID] FROM [PS_GameData].[dbo].[CharItems] WHERE [CharID] = '".$this->cid."' AND [Slot] = '$slot'");
		$this->res = $this->sql->execute();
		
		if ($this->sql->rowCount() != 1)
			return false;
		
		$this->sql = $this->conn->prepare("DELETE FROM [PS_GameData].[dbo].[CharItems] WHERE [CharID] = '".$this->cid."' AND [Slot] = '$slot'");
		$this->res = $this->sql->execute();
	...
	public function deleteAll(){
		$this->sql = $this->conn->prepare("DELETE FROM [PS_GameData].[dbo].[CharItems] WHERE [CharID] = '".$this->cid."'");
		$this->res = $this->sql->execute();
