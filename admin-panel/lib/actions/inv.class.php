...
Class invAction{
	...
	public function execute(){
		...
		if (isset($_GET['uid'])){
			...
			$this->item = new item($_GET['uid'], 0);
			$this->char = new char($this->item->getCharID());
				...
				switch($_GET['do']){
					case 'halt':
						if (isset($_POST['halt'])){
							...
							echo '<div Class="error">Please provide a correct value for durability.</div>';
						break;
					case 'count':
						if (isset($_POST['count'])){
							...
							echo '<div Class="error">Please provide a correct count valur.</div>';
						break;
					case 'craft':
						if (isset($_POST['stat'])){
							$this->error = $this->item->changeCraftname($_GET['stat'], $_POST['stat']);
							if ($this->error == 1)
								echo '<div Class="success">EP-4 Stats (OJ) of '.$this->item->getItemName().' were changed.</div>';
							else
								echo '<div Class="error">'.$this->error.'</div>';
						}
						else
							echo '<div Class="error">Please provide a OJ value.</div>';
						break;
					case 'unlinklapis':
						$this->error = $this->item->extractLapis($_GET['slot']);
						if ($this->error == 1)
							echo '<div Class="success">Lapis (Slot: '.$_GET['slot'].') successfully extracted from '.$this->item->getItemName().'</div>';
						else
							echo '<div Class="error">'.$this->error.'</div>';
						break;
					case 'linklapis':
						if (isset($_POST['lapis'])){
							...
							if ($this->error == 1)
								echo '<div Class="success">Lapis linked to '.$this->item->getItemName().'.</div>';
							else
								echo '<div Class="error">'.$this->error.'</div>';
						}
						else
					...
					case 'delete':
						...
						<!-- 
						document.location.href = "index.php?action=char&CharID=<?php echo $this->char->getCharID();?>"; 
						// --> 
						</script>
						<?php
						exit();
			...
			if ($this->item->getBag() == 0)
				$this->infoArr['Bag'] = 'Wearing';
			else
				$this->infoArr['Bag'] = $this->item->getBag();
			...
			$this->infoArr['LapisDia'] = '';
				...
				$this->infoArr['LapisDia']			.= '<div id="link1dia" title="Link a lapis">Please put the lapis ID that you want to link to the item "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.BIntString($this->item->getUID()).'") in slot 1.<form action="index.php?action=inv&amp;uid='.BIntString($this->item->getUID()).'&amp;do=linklapis&amp;slot=1" method="POST" id="link1diaform"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis2() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="link2dia" title="Link a lapis">Please put the lapis ID that you want to link to the item "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.BIntString($this->item->getUID()).'") in slot 2.<form action="index.php?action=inv&amp;uid='.BIntString($this->item->getUID()).'&amp;do=linklapis&amp;slot=1" method="POST" id="link2diaform"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis4() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="link4dia" title="Link a lapis">Please put the lapis ID that you want to link to the item "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.BIntString($this->item->getUID()).'") in slot 4.<form action="index.php?action=inv&amp;uid='.BIntString($this->item->getUID()).'&amp;do=linklapis&amp;slot=1" method="POST" id="link4diaform"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis5() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="link5dia" title="Link a lapis">Please put the lapis ID that you want to link to the item "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.BIntString($this->item->getUID()).'") in slot 5.<form action="index.php?action=inv&amp;uid='.BIntString($this->item->getUID()).'&amp;do=linklapis&amp;slot=1" method="POST" id="link5diaform"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis6() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="link6dia" title="Link a lapis">Please put the lapis ID that you want to link to the item "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.BIntString($this->item->getUID()).'") in slot 6.<form action="index.php?action=inv&amp;uid='.BIntString($this->item->getUID()).'&amp;do=linklapis&amp;slot=6" method="POST" id="link6diaform"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}			
			
			echo '<table>';
			echo '<tr><td>UserUID: </td><td>'.$this->user->getUserUID().'</td><td></td></tr>';
			echo '<tr><td>CharID: </td><td>'.$this->char->getCharID().'</td><td></td></tr>';
			echo '<tr><td>Account: </td><td><a href="index.php?action=user&amp;UserUID='.$this->user->getUserUID().'">'.$this->user->getName().'</a></td><td></td></tr>';
			echo '<tr><td>Character: </td><td><a href="index.php?action=char&amp;CharID='.$this->char->getCharID().'">'.$this->char->getName().'</a></td><td></td></tr>';
			...
			echo '<tr><td>Bag: </td><td>'.$this->infoArr['Bag'].'</td><td></td></tr>';
			...
			echo '<tr><td>Del: </td><td></td><td><a  id="del">Delete item</a></td></tr>';
			echo '</table>';
			...
			$(function(){
				...
				$("#unlink1dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=1';
							$(this).dialog('close');
						},
				...
				$("#unlink2dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=2';
							$(this).dialog('close');
						},
				...
				$("#unlink3dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=3';
							$(this).dialog('close');
						},
				...
				$("#unlink4dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=4';
							$(this).dialog('close');
						},
				...
				$("#unlink5dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=5';
							$(this).dialog('close');
						},
				...
				$("#unlink6dia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=unlinklapis&slot=6';
							$(this).dialog('close');
						},
				...
				$("#deldia").dialog({
					...
					buttons: {
						Yes: function() {
							document.location.href = 'index.php?action=inv&uid=<?php echo BIntString($this->item->getUID());?>&do=delete';
							$(this).dialog('close');
						},
			...
			<div id="haltdia" title="Durability Change">Please provide de new durability '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=halt" method="POST" id="haltdiaform"><br><input type="text" name="halt" maxlength=4 size=4></form></div>
			<div id="countdia" title="Count Change">Please provide the new count for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=count" method="POST" id=""><br><input type="text" name="count" maxlength=4 size=4></form></div>
			<div id="strdia" title="Str Change">Please provide the new STR for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Str" method="POST" id="strdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="dexdia" title="Dex Change">Please provide the new DEX for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Dex" method="POST" id="dexdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="recdia" title="Rec Change">Please provide the new REC for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Rec" method="POST" id="recdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="intdia" title="Int Change">Pleace provide the new INT for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Int" method="POST" id="intdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="wisdia" title="Wis Change">Please provide the new WIS for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Wis" method="POST" id="wisdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="lucdia" title="Luc Change">Please provide the new LUC for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Luc" method="POST"id="lucdiaform"><br><input type="text" name="stat" maxlength=2 size=2></form></div>
			<div id="hpdia" title="LP Change">Please provide the new HP for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=HP" method="POST" id="hpdiaform"><br><input type="text" name="stat" maxlength=2 size=4>00</form></div>
			<div id="mpdia" title="MP Change">Please provide the new MP for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=MP" method="POST" id="mpdiaform"><br><input type="text" name="stat" maxlength=2 size=4>00</form></div>
			<div id="spdia" title="SP Change">Please provide the new SP for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=SP" method="POST" id="spdiaform"><br><input type="text" name="stat" maxlength=2 size=4>00</form></div>
			<div id="lapisadia" title="Lapisia">Please provide the new lapisia value for '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>) <form action="index.php?action=inv&amp;uid=<?php echo BIntString($this->item->getUID());?>&amp;do=craft&amp;stat=Lapisa" method="POST" id="lapisadiaform"><br><input type="text" name="stat" maxlength=2 size=4><br>0-20 for weapons<br>50-70 for armors</form></div>
			<div id="deldia" title="Item Delete">Do you want to delete this item '<?php echo $this->item->getItemName();?>' (Character: <?php echo $this->char->getName();?>, ItemUID: <?php echo BIntString($this->item->getUID());?>)?</div>
