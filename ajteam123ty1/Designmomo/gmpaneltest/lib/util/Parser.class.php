...
class Parser {
	...
	public static function datum($zeit){
		$tag	= date('l', $zeit);
		$tage	= array('Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche');
	...
	public static function lapis($gem){
		if (!self::gint($gem))
			throw new SystemException('Gem doit être de type entier.', 0, 0, __FILE__, __LINE__);
			
		if ($gem > 255)
			throw new SystemException('Gem ne peut pas être supérieure à 255.', 0, 0, __FILE__, __LINE__);
	...
	public static function Zeitspanne($i){
		...
		return $minuten.' Minutes et  '.$i.' Seconde';
	}
