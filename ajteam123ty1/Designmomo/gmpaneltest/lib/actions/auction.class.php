		...
		if (!isset($_GET['mid']) && !isset($_POST['Suche'])){
			if (isset($_GET['top'])){
				if (Parser::gint($_GET['top']))
					$this->top = Top::Auctions($_GET['top']);
				else
					$this->top = Top::Auctions(500);
			}
			else
				$this->top = Top::Auctions(500);
			...
			echo '<table>';
			echo '<tr>';
			echo '<th>Market-ID</th><th>Char-ID</th><th>User-UID</th><th>Char-Name</th><th>nom d utilisateur</th><th>Item-Name</th><th>fonctionne jusqu à</th>';
			echo '</tr>';
			...
			echo '<tr><td>nombre: </td><td>'.$this->auction->getCount().'</td></tr>';
			echo '<tr><td>Personnage: </td><td><a href="index.php?àction=char&amp;CharID='.$this->auction->getCharID().'">'.$this->char->getName().'</a></td></tr>';
			...
			echo '<tr><td>Prix instantanée: </td><td>'.Parser::Zahl($this->auction->getDirectMoney()).'</td></tr>';
			echo '<tr><td>coûts de ventes: </td><td>'.Parser::Zahl($this->auction->getKosten()).'</td></tr>';
			echo '<tr><td>Prix  maximum: </td><td>'.$this->infoArr['Bieter'].'</td></tr>';
			echo '<tr><td>Enchère gagnante: </td><td>'.$this->auction->getGebot().'</td></tr>';
			echo '<tr><td>Fin: </td><td>'.Parser::Datum($this->auction->getEnde()).'</td></tr>';
			echo '</table>';
