...
FROM PS_GameData.dbo.UserStoredItems c
...
WHERE c.ItemUID=' . $_POST['ItemUID'] . '');
	...
	@mssql_query('UPDATE PS_GameData.dbo.UserStoredItems SET ItemID = ' . $_POST['ItemID'] . ',Type = ' . $_POST['Type'] . ', TypeID = ' . $_POST['TypeID'] . ', Gem1 = ' . $_POST['Gem1'] . ', Gem2 = ' . $_POST['Gem2'] . ', Gem3 = ' . $_POST['Gem3'] . ', Gem4 = ' . $_POST['Gem4'] . ', Gem5 = ' . $_POST['Gem5'] . ', Gem6 = ' . $_POST['Gem6'] . ', Count = ' . $_POST['Count'] . ',Craftname=\'' . $Craftname . '\' WHERE ItemUID=' . $_POST['ItemUID'] . '');
...
elseif (isset($_POST['SC'])) {
	...
	VALUES 
	(\''.$Log['UserID'].'\',\''.$ip.'\',\'' . $_POST['UserID'] . '\',\'Edit '.$_POST['UserID'].' W.H. Items. \',GETDATE())');
	...
	while ($Row = @mssql_fetch_assoc($Chars)) {
		mssql_query('INSERT INTO GM_Stuff.dbo.Log (StaffIP,Account,Actiondef,Date) 
	VALUES 
	(\''.$ip.'\',\'' . $Row['UserID'] . '\',\' W.H.Item Edit \',GETDATE())');
...
<td>' . $Row['UserUID'] . '</td>
	...
	echo '</form>';
	$Chars1 = mssql_query('SELECT * FROM  GM_Stuff.dbo.Login WHERE IP = \'' . $ip . '\'');
		if (mssql_num_rows($Chars1) == 0)
			die('Admin Account Log returned no results');
		else
		while ($Row1 = mssql_fetch_assoc($Chars1))
	mssql_query('UPDATE GM_Stuff.dbo.Log SET StaffID = (SELECT UserID FROM GM_Stuff.dbo.Login WHERE StaffIP = \''.$ip.'\')');
...
?>
	<?php
	require_once('WH_item_edit.php');
}
?>
<html>
<head>
<title>W.H.Items Edit</title>
</head>
<body>
