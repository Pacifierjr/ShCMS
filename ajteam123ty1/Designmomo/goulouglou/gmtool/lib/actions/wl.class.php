	...
	public function execute(){
		if (isset($_GET['uid'])){
			if (!Parser::gint($_GET['uid']))
				throw new SystemException('ItemUID doit être de type entier', 0, 0, __FILE__, __LINE__);
				...
				switch($_GET['do']){
					case 'halt':
						if (isset($_POST['halt'])){
							...
							if ($this->error == 1)
								echo '<div class="success">la durabilité des Items '.$this->item->getItemName().' a été changé avec succès.</div>';
							else
					...
					case 'count':
						if (isset($_POST['count'])){
							...
							if ($this->error == 1)
								echo '<div class="success">le nombre des Items '.$this->item->getItemName().' a été changé avec succès.</div>';
							else
								echo '<div class="error">'.$this->error.'</div>';
						}
						else
					...
					case 'craft':
						if (isset($_POST['stat'])){
							...
							if ($this->error == 1)
								echo '<div class="success">EP-6 Stats des Items '.$this->item->getItemName().'changé avec succès.</div>';
							else
								echo '<div class="error">'.$this->error.'</div>';
						}
						else
					...
					case 'unlinklapis':
						...
						if ($this->error == 1)
							echo '<div class="success">Lapis (Slot: '.$_GET['slot'].') avec succès à partir de l élément '.$this->item->getItemName().' entfernt.</div>';
						else
							echo '<div class="error">'.$this->error.'</div>';
						break;
					case 'linklapis':
			...
			if ($this->item->getLapis1() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink1dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis1']['Name']	= '';
				$this->infoArr['Lapis1']['Link']	= '<a href="javascript::void();" id="link1">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link1dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=1" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis2() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink2dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis2']['Name']	= '';
				$this->infoArr['Lapis2']['Link']	= '<a href="javascript::void();" id="link2">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link2dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=2" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis3() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink3dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis3']['Name']	= '';
				$this->infoArr['Lapis3']['Link']	= '<a href="javascript::void();" id="link3">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link3dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=3" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis4() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink4dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis4']['Name']	= '';
				$this->infoArr['Lapis4']['Link']	= '<a href="javascript::void();" id="link4">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link4dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=4" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis5() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink5dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis5']['Name']	= '';
				$this->infoArr['Lapis5']['Link']	= '<a href="javascript::void();" id="link5">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link5dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=5" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			if ($this->item->getLapis6() != 0){
				...
				$this->infoArr['LapisDia']			.= '<div id="unlink6dia" title="Lapis supprimer">Voulez-vous Lapis "'.$this->infoArr['TempLapis']->getName().'" des Items "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vraiment retirer?</div>';
			} else {
				$this->infoArr['Lapis6']['Name']	= '';
				$this->infoArr['Lapis6']['Link']	= '<a href="javascript::void();" id="link6">vous sertissez Lapis</a>';
				$this->infoArr['LapisDia']			.= '<div id="link6dia" title="Lapis vous sertissez "> Entrez les Lapises TypeID de vous dans l élément "'.$this->item->getItemName().'" (Account: "'.$this->user->getName().'", ItemUID: "'.$this->item->getUID().'") vouloir vous sertissez, une.<form action="index.php?action=wl&amp;uid='.$this->item->getUID().'&amp;do=linklapis&amp;slot=6" method="POST"><br><input type="text" name="lapis" maxlength=3 size=3></form></div>';
			}
			...
			echo '<tr><td>durabiliter: </td><td>'.Parser::Zahl($this->item->getQuality()).'</td><td><a href="javascript::void();" id="halt">changer la durabiliter</a></td></tr>';
			echo '<tr><td>Max. durabiliter: </td><td>'.Parser::Zahl($this->item->getMaxQuality()).'</td><td></td></tr>';
			...
			echo '<tr><td>Str: </td><td>'.(int)$this->item->getStr().'</td><td><a href="javascript::void();" id="str">Str Modifier</a></td></tr>';
			echo '<tr><td>Dex: </td><td>'.(int)$this->item->getDex().'</td><td><a href="javascript::void();" id="dex">Dex Modifier</a></td></tr>';
			echo '<tr><td>Rec: </td><td>'.(int)$this->item->getRec().'</td><td><a href="javascript::void();" id="rec">Rec Modifier</a></td></tr>';
			echo '<tr><td>Int: </td><td>'.(int)$this->item->getInt().'</td><td><a href="javascript::void();" id="int">Int Modifier</a></td></tr>';
			echo '<tr><td>Wis: </td><td>'.(int)$this->item->getWis().'</td><td><a href="javascript::void();" id="wis">Wis Modifier</a></td></tr>';
			echo '<tr><td>Cha: </td><td>'.(int)$this->item->getLuc().'</td><td><a href="javascript::void();" id="luc">Cha Modifier</a></td></tr>';
			echo '<tr><td>HP: </td><td>'.Parser::Zahl((int)$this->item->getHP() * 100).'</td><td><a href="javascript::void();" id="hp">HP Modifier</a></td></tr>';
			echo '<tr><td>MP: </td><td>'.Parser::Zahl((int)$this->item->getMP() * 100).'</td><td><a href="javascript::void();" id="mp">MP Modifier</a></td></tr>';
			echo '<tr><td>SP: </td><td>'.Parser::Zahl((int)$this->item->getSP() * 100).'</td><td><a href="javascript::void();" id="sp">SP Modifier</a></td></tr>';
			...
			$(function(){
				...
				$("#unlink1dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=1';
							$(this).dialog('close');
						},
				...
				$("#unlink2dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=2';
							$(this).dialog('close');
						},
				...
				$("#unlink3dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=3';
							$(this).dialog('close');
						},
				...
				$("#unlink4dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=4';
							$(this).dialog('close');
						},
				...
				$("#unlink5dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=5';
							$(this).dialog('close');
						},
				...
				$("#unlink6dia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=unlinklapis&slot=6';
							$(this).dialog('close');
						},
				...
				$("#deldia").dialog({
					...
					buttons: {
						Ja: function() {
							document.location.href = 'index.php?action=wl&uid=<?php echo $this->item->getUID();?>&do=delete';
							$(this).dialog('close');
						},
