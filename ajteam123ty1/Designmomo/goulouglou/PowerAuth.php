...
//     <?php $PowerAuth_password = "Define Password Here"; include "PowerAuth.php"; ?/>  <---(remove "/")
...
//  5. To apply CSS styles, modify the loginBox class and its children.The default stylesheet is at the bottom of this file.
class CAuth
{
	protected $password;
	protected $maxTries;
...
if (!isset($PowerAuth_maxPasswordAttempts))
	$PowerAuth_maxPasswordAttempts = 6; //number of attempts to allow before locking user out
//if lockout time var is not defined, set its value to the default
if (!isset($PowerAuth_lockoutTime))
	$PowerAuth_lockoutTime = 30; //number of seconds to wait if locked out
//if lockout time increment var is not defined, set its value to the default
if (!isset($PowerAuth_lockoutTimeIncrement))
	$PowerAuth_lockoutTimeIncrement = 15; // number of seconds to increase lockout time in each succesfull lockout
